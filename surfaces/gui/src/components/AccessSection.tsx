...
export function AccessSection({
  ...
  return (
    <section className="rail-section" ref={rootEl} data-testid="access-section">
      ...
      {open && (
        <div className="rail-section-body" role="region" aria-label="会话访问权限">
          {connectFor ? (
            <ConnectInline
              c={connectFor}
              cloud={cloud}
  ...
  return (
    <div>
      ...
      <p className="text-[10.5px] text-faint mt-2 leading-snug">
        连接后 {c.title} 将对你的所有数字同事可用 —— 本列表中的开关仅控制当前会话。
      </p>
    </div>
  );
}
  ...
  return (
    <div>
            ...
            <div className="flex items-center gap-1.5 py-1" key={s.channel}>
              ...
              {s.collision && (
                <span
                  ...
                  title="该频道同时是本会话收件箱路由的目标 —— 入站与出站会冲突。"
                >
                  ⚠
                </span>
              )}
      ...
      <p className="text-[10.5px] text-faint mt-1.5 leading-snug">
        智能体将接收发往这些频道的消息。移除某个频道会停止本会话的监听 —— 连接器仍保持连接。
      </p>
    </div>
  );
}
