...
| F13 | 企业内部系统连接器（OA/工单等，首个） 🟡**模板与指南已交付，接第一个真实系统待内网接口** —— 声明式 HTTP→MCP 桥（路线 A，零上游改动）+ 原生描述符模板（路线 B，逐工具审批）；见 [CONNECTOR_GUIDE.md](CONNECTOR_GUIDE.md) | P2 | 代码级 | 完成一个真实内部系统的读写操作 |
